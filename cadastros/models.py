...
class Membro(models.Model):
    # ... (os campos CARGO_CHOICES, SEXO_CHOICES, etc., continuam os mesmos) ...
    CARGO_CHOICES = [
    ...
    especialidades_conquistadas = models.ManyToManyField(Especialidade, through='MembroEspecialidadeConquistada', blank=True)
    classes_conquistadas = models.ManyToManyField(Classe, through='MembroClasseConquistada', blank=True, related_name='concluintes')
    ...
    @property
    def idade(self):
        # 1. ADICIONADA UMA VERIFICAÇÃO AQUI
    ...
    def clean(self):
        ...
        # 2. ADICIONADA UMA VERIFICAÇÃO AQUI
        # A validação só acontece se a idade não for nula
        if self.idade is not None and self.idade > 16 and self.cargo in cargos_juvenis:
            raise ValidationError(
                f"Membros com mais de 16 anos não podem ter o cargo de '{self.get_cargo_display()}'."
            )
...
class Patrimonio(models.Model):
    ...
    responsavel = models.ForeignKey(Membro, on_delete=models.SET_NULL, null=True, blank=True, help_text="Membro responsável pelo Item (opcional)")
...
class FichaMedica(models.Model):
    ...
    plano_de_saude = models.CharField(max_length=100, blank=True, verbose_name="Plano de Saúde (Nome e número)")
